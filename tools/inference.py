...
"""
Inference script for Uformer-based image restoration.
...
Usage example:
    python inference.py \
        ...
        --tile 256 --tile-overlap 32 --fp16

Notes:
- Expects RGB images.
- Normalizes to [0,1].
- Will try to infer embed_dim from the loaded weights automatically.
- If your model was trained with different depths/win-size, pass them via args.
...
# Uformer code
###########################
# Paste your full Uformer implementation here (the big code you sent).
# Make sure classes like: Uformer, InputProj, OutputProj, etc. are present.
# Keep the __main__ test of that file OUT—this script is the entry point.
#
...
# -------------------------
# Helpers: state_dict & model builder
# -------------------------
def extract_state_dict(weights_path: str) -> Dict[str, torch.Tensor]:
    """Load a .ckpt/.pth and return a cleaned PyTorch state_dict."""
    ...
    for k, v in state_dict.items():
        nk = k
        for pfx in ("model_restoration.", "model.", "net.", "module."):
...
def infer_embed_dim_from_sd(sd: Dict[str, torch.Tensor], fallback: int = 32) -> int:
    """Try to guess embed_dim from the input_proj weight shape."""
    for k, v in sd.items():
        # Typical key: input_proj.proj.0.weight -> [embed_dim, 3, 3, 3]
        if "input_proj.proj.0.weight" in k and v.ndim == 4:
            return int(v.shape[0])
    return fallback


def parse_depths(s: str) -> List[int]:
    """Parse depths like '2,2,2,2,2,2,2,2,2'."""
    parts = [p.strip() for p in s.split(",") if p.strip()]
    return [int(p) for p in parts]


def build_uformer_for_infer(
    ...
    if embed_dim_override is not None:
        ...
        embed_dim = infer_embed_dim_from_sd(sd, fallback=32)

    # The Uformer class must exist in the pasted section above
    model = Uformer(
        img_size=img_size,
        embed_dim=embed_dim,
        depths=depths,
...
def load_image_rgb(path: str) -> np.ndarray:
    """Load an image as HxWxC in RGB, float32 in [0,1]."""
    img = Image.open(path).convert("RGB")
    arr = np.array(img).astype(np.float32) / 255.0
    return arr


def save_image_rgb(arr: np.ndarray, out_path: str) -> None:
    """Save HxWxC float array in [0,1] as RGB PNG/JPG by extension."""
...
# Tiling utilities
# -------------------------
def pad_to_multiple(h: int, w: int, multiple: int) -> Tuple[int, int, int, int]:
    """Return padding (top,bottom,left,right) so that (h+top+bottom) and (w+left+right)
    are divisible by `multiple`."""
...
def reflect_pad(img: torch.Tensor, pad: Tuple[int, int, int, int]) -> torch.Tensor:
    """img: (1,3,H,W). pad=(top,bottom,left,right)"""
    t, b, l, r = pad
    if t or b or l or r:
        img = torch.nn.functional.pad(img, (l, r, t, b), mode="reflect")
    return img
...
def sliding_window_inference(
    ...
    """
    Apply sliding-window inference on a single image tensor (1,3,H,W) -> (1,3,H,W).

    The network is fully-convolutional, so tile padding is handled by reflection.
    We accumulate outputs and an overlap count to average overlaps.
    """
    model.eval()
    b, c, h, w = img.shape
    assert b == 1, "Use batch=1 per image for this function."
    ...
    # Reflect pad so that we can slide perfect grid
    pad = pad_to_multiple(h, w, stride)
    img_pad = reflect_pad(img, pad)
    _, _, H, W = img_pad.shape

    # Create accumulators
    output = torch.zeros_like(img_pad)
    count = torch.zeros_like(img_pad)
        ...
        for y in y_positions:
            for x in x_positions:
                ...
                if fp16:
                    ...
                    pred = model(patch)
                output[:, :, y:y + tile, x:x + tile] += pred
                count[:, :, y:y + tile, x:x + tile] += 1.0

    output = output / torch.clamp_min(count, 1e-6)

    # remove padding
    t, b, l, r = pad
    output = output[:, :, t:output.shape[2]-b if b > 0 else output.shape[2],
                         l:output.shape[3]-r if r > 0 else output.shape[3]]

    # Crop back to original size (in case rounding weirdness)
    output = output[:, :, :h, :w]
    return output
...
def run_on_image(
    ...
    arr = load_image_rgb(path_in)  # H,W,C in [0,1]
    ...
    save_image_rgb(out, path_out)
    print(f"[OK] {os.path.basename(path_in)} -> {path_out}")


def gather_images(input_path: str) -> List[str]:
...
def main():
    parser = argparse.ArgumentParser("Uformer Inference")
    parser.add_argument("--weights", required=True, type=str, help="Path to .ckpt or .pth")
    parser.add_argument("--input", required=True, type=str, help="Image file or folder")
    parser.add_argument("--output", required=True, type=str, help="Output folder")
    parser.add_argument("--tile", type=int, default=256, help="Tile size (for sliding window)")
    parser.add_argument("--tile-overlap", type=int, default=32, help="Overlap (<= tile)")
    parser.add_argument("--fp16", action="store_true", help="Use autocast fp16 if available")
    parser.add_argument("--device", type=str, default="cuda", help="cuda / cuda:0 / cpu")
    parser.add_argument("--img-size", type=int, default=256, help="Model img_size (use your train value)")
    parser.add_argument("--win-size", type=int, default=8, help="Uformer window size")
    parser.add_argument("--depths", type=str, default="2,2,2,2,2,2,2,2,2",
                        help="Comma list for depths, e.g. '2,2,2,2,2,2,2,2,2'")
    parser.add_argument("--embed-dim", type=int, default=None,
                        help="Override embed_dim. If None, infer from weights")
    parser.add_argument("--token-projection", type=str, default="linear", choices=["linear", "conv", "linear_concat"])
    parser.add_argument("--token-mlp", type=str, default="leff", choices=["leff", "ffn"])
    parser.add_argument("--se-layer", action="store_true", help="Enable SELayer in attention block")
    ...
    # Load weights
    sd = extract_state_dict(args.weights)

    # Build model
    depths_list = parse_depths(args.depths)
    model = build_uformer_for_infer(
        sd=sd,
    ...
    # (Optional) convert to half—let autocast control precision instead
    # model.half()  # Not necessary; autocast handles it

    # Collect images
    inputs = gather_images(args.input)
    if not inputs:
    ...
    # Process
    for p in inputs:
        out_name = os.path.splitext(os.path.basename(p))[0] + "_restored.png"
