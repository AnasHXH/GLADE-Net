...
#       - Level 1: residual at original resolution.
#       - Level 2: residual computed one octave coarser, then upsampled to original size.
#     Returns (L1, L2, combined=L1+L2).
#     """
#     def __init__(self):
#         super().__init__()
#         k = torch.Tensor([[.05, .25, .4, .25, .05]])
#         kernel = torch.matmul(k.t(), k).unsqueeze(0).repeat(3, 1, 1, 1)  # 3-channel separable Gaussian
#         self.register_buffer('kernel', kernel)

#     def conv_gauss(self, img):
#         # replicate-padding so size is preserved through conv
#         _, _, kh, kw = self.kernel.shape
#         img = F.pad(img, (kw // 2, kw // 2, kh // 2, kh // 2), mode='replicate')
#         return F.conv2d(img, self.kernel, groups=img.shape[1])

#     def downsample(self, x):
#         # 2x downsample (keep every other pixel)
#         return x[:, :, ::2, ::2]

#     def upsample_like_prev(self, x_down, like):
#         """
#         Upsample a downsampled feature map back to the spatial size of `like`
#         using zero-insertion (×4 energy) followed by Gaussian smoothing
#         (matches your original implementation).
#         """
#         up_zeros = torch.zeros_like(like)
#         up_zeros[:, :, ::2, ::2] = x_down * 4.0
#         return self.conv_gauss(up_zeros)

#     def forward(self, img):
#         # --- Level 1 (full-res residual) ---
#         g0 = self.conv_gauss(img)        # blur
#         d1 = self.downsample(g0)         # downsample to half res
#         u1 = self.upsample_like_prev(d1, g0)  # upsample back to full res
#         L1 = img - u1                    # Laplacian residual at level 1 (full size)

#         # --- Level 2 (one octave coarser) ---
#         g1 = self.conv_gauss(d1)         # blur at half res
#         d2 = self.downsample(g1)         # downsample to quarter res
#         u2_half = self.upsample_like_prev(d2, g1)   # upsample to half res
#         L2_half = d1 - u2_half                          # residual at half res
#         L2 = self.upsample_like_prev(L2_half, g0)       # bring L2 to full res
